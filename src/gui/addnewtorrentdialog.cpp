...
AddNewTorrentDialog::AddNewTorrentDialog(const BitTorrent::TorrentDescriptor &torrentDescr
    ...
    , m_storeSizeFilterEnabled {SETTINGS_KEY(u"SizeFilterEnabled"_s)}
    , m_storeSizeFilterValue {SETTINGS_KEY(u"SizeFilterValue"_s)}
    , m_storeSizeFilterUnit {SETTINGS_KEY(u"SizeFilterUnit"_s)}
{
    m_ui->setupUi(this);
    ...
    // Size filter: enable/disable controls and auto-apply when settings change
    connect(m_ui->checkBoxSizeFilter, &QCheckBox::toggled, this, [this](const bool checked)
    {
        m_ui->lineEditSizeFilter->setEnabled(checked);
        m_ui->comboBoxSizeUnit->setEnabled(checked);
        if (checked)
            applySizeFilter();
    });
    connect(m_ui->lineEditSizeFilter, &QLineEdit::textChanged, this, [this]()
    {
        if (m_ui->checkBoxSizeFilter->isChecked())
            applySizeFilter();
    });
    connect(m_ui->comboBoxSizeUnit, qOverload<int>(&QComboBox::currentIndexChanged), this, [this]()
    {
        if (m_ui->checkBoxSizeFilter->isChecked())
            applySizeFilter();
    });
...
void AddNewTorrentDialog::loadState()
{
    ...
    // Restore size filter settings
    const QString savedSizeValue = m_storeSizeFilterValue.get(u"100"_s);
    const int savedSizeUnit = m_storeSizeFilterUnit.get(1); // default: MB (index 1)
    const bool savedSizeFilterEnabled = m_storeSizeFilterEnabled.get(false);

    m_ui->lineEditSizeFilter->setText(savedSizeValue);
    m_ui->comboBoxSizeUnit->setCurrentIndex(qBound(0, savedSizeUnit, m_ui->comboBoxSizeUnit->count() - 1));
    // Set checkbox last so the toggled signal fires with restored value/unit already applied
    m_ui->checkBoxSizeFilter->setChecked(savedSizeFilterEnabled);
}
...
void AddNewTorrentDialog::saveState()
{
    ...
    m_storeSizeFilterEnabled = m_ui->checkBoxSizeFilter->isChecked();
    m_storeSizeFilterValue = m_ui->lineEditSizeFilter->text();
    m_storeSizeFilterUnit = m_ui->comboBoxSizeUnit->currentIndex();
}
...
void AddNewTorrentDialog::applySizeFilter()
{
    const QString sizeText = m_ui->lineEditSizeFilter->text().trimmed();
    bool ok = false;
    const double sizeValue = sizeText.toDouble(&ok);
    if (!ok || sizeValue < 0)
        return;

    // Multipliers: index 0=KB, 1=MB, 2=GB, 3=TB
    static const qulonglong unitMultipliers[] = {
        Q_UINT64_C(1024),                    // KB
        Q_UINT64_C(1024) * 1024,             // MB
        Q_UINT64_C(1024) * 1024 * 1024,      // GB
        Q_UINT64_C(1024) * 1024 * 1024 * 1024 // TB
    };

    const int unitIndex = m_ui->comboBoxSizeUnit->currentIndex();
    const qulonglong multiplier = unitMultipliers[qBound(0, unitIndex, 3)];
    const qulonglong minSizeBytes = static_cast<qulonglong>(sizeValue * multiplier);

    m_ui->contentTreeView->checkByMinSize(minSizeBytes);
}
